...
body{{display:flex;align-items:center;justify-content:center}} .shell{{position:relative;width:calc(100% - 36px);height:calc(100% - 40px);min-width:330px;min-height:460px;overflow:hidden;background:rgba(14,25,72,.76);border:1.4px solid rgba(203,236,255,.66);box-shadow:0 22px 70px rgba(4,7,35,.55), inset 0 1px 0 rgba(255,255,255,.25);backdrop-filter:blur(30px) saturate(145%)}}
...
.top{{position:absolute;top:45px;left:46px;right:42px}} h1{{font-size:22px;margin:0 0 6px;font-weight:700;letter-spacing:-.5px}} .sub{{font-size:12px;color:#d5e4ff;opacity:.9}} .close{{position:absolute;right:0;top:-5px;width:40px;height:40px;border-radius:50%;border:1px solid rgba(255,255,255,.38);background:rgba(255,255,255,.14);color:white;font-size:28px;line-height:35px;cursor:pointer}}
...
.top{{top:31px}} .switch{{top:112px}} .main{{top:202px}} .claude-mode .main{{top:194px}} .claude-mode .weekly{{margin-top:13px;padding-top:9px}} .claude-mode .weekly .bar{{margin-top:6px}}
@media (max-height:490px){{.top{{top:22px;left:34px;right:32px}}.switch{{top:93px;left:34px;right:34px;height:56px}}.main,.claude-mode .main{{top:163px;left:34px;right:34px}}.percent{{font-size:64px;line-height:68px}}.claude-mode .percent{{font-size:50px;line-height:54px}}.bar{{margin-top:14px}}.updated{{bottom:26px}}}}
</style></head><body><div id="shell" class="shell {shape}"><div class="shine"></div><div class="top"><h1>AI Usage</h1><div class="sub">Your limits, beautifully in view</div><button class="close" onclick="document.title='close'">×</button></div><div class="switch"><button id="chat" class="choice active" onclick="pick('ChatGPT')"><b>✦ ChatGPT</b>Weekly</button><button id="claude" class="choice" onclick="pick('Claude')"><b>✺ Claude</b>5-hour + weekly</button></div><div class="main"><div id="eyebrow" class="eyebrow">Weekly usage</div><div id="percent" class="percent">—</div><div id="reset" class="reset">Waiting for sync</div><div class="bar"><div id="fill" class="fill"></div></div><div id="caption" class="caption">of weekly allowance used</div><div id="weekly" class="weekly"><div class="weekly-head"><span>Weekly limit</span><b id="weekly-used">—</b></div><div id="weekly-reset" class="reset">Waiting for sync</div><div class="bar"><div id="weekly-fill" class="fill"></div></div></div></div><svg class="wave" viewBox="0 0 350 54" preserveAspectRatio="none"><path d="M0,34 C35,15 58,49 98,29 S154,51 201,20 S276,43 350,8" fill="none" stroke="#5bf4e1" stroke-width="3"/></svg><div id="updated" class="updated">Local • private • always in reach</div><div class="grip"></div></div><script>let provider='ChatGPT';function pick(p){{provider=p;document.getElementById('chat').classList.toggle('active',p==='ChatGPT');document.getElementById('claude').classList.toggle('active',p==='Claude');render(window.data||{{}})}}function pct(e){{return Number.isFinite(e?.used)?e.used:(Number.isFinite(e?.remaining)?100-e.remaining:null)}}function render(d){{window.data=d;let w=d?.providers?.[provider]||{{}};let e=w[provider==='Claude'?'5-hour':'Weekly']||{{}};let used=pct(e);document.getElementById('shell').classList.toggle('claude-mode',provider==='Claude');document.getElementById('eyebrow').textContent=provider==='Claude'?'Current session':'Weekly usage';document.getElementById('percent').textContent=used===null?'—':Math.round(used)+'%';document.getElementById('fill').style.width=(used||0)+'%';document.getElementById('reset').textContent=e.reset_label||'Waiting for sync';document.getElementById('caption').textContent=provider==='Claude'?'of 5-hour session used':'of weekly allowance used';let weekly=w['Weekly']||{{}};let wu=pct(weekly);document.getElementById('weekly-used').textContent=wu===null?'—':Math.round(wu)+'% used';document.getElementById('weekly-reset').textContent=weekly.reset_label||'Waiting for sync';document.getElementById('weekly-fill').style.width=(wu||0)+'%';document.getElementById('updated').textContent=e.updated_at?'Last updated '+new Date(e.updated_at).toLocaleTimeString():'Local • private • always in reach'}};document.addEventListener('mousedown',e=>{{if(e.button!==0||e.target.closest('button'))return;const m=16,x=e.clientX,y=e.clientY,w=innerWidth,h=innerHeight;let edge='';if(y<m)edge+='N';else if(y>h-m)edge+='S';if(x<m)edge+=edge?'_W':'W';else if(x>w-m)edge+=edge?'_E':'E';if(edge){{e.preventDefault();document.title=`resize:${{edge}}:${{Math.round(e.screenX)}}:${{Math.round(e.screenY)}}`;}}else if(y<130){{e.preventDefault();document.title=`move:0:${{Math.round(e.screenX)}}:${{Math.round(e.screenY)}}`;}}}},true);</script></body></html>'''
...
class App:
    def __init__(self, theme):
        ...
        self.web.load_html(html(theme), 'file:///'); self.window.show_all(); GLib.timeout_add(1000,self.tick)
    ...
    def tick(self):
        ...
        while not EVENTS.empty():
            p=EVENTS.get(); data=json.loads(STATE.read_text()); provider=p['provider']
            for name,val in p.get('windows',{}).items():
                if name not in data['providers'][provider]: continue
                if isinstance(val.get('used'),(int,float)): data['providers'][provider][name]['used']=val['used'];data['providers'][provider][name]['remaining']=100-val['used']
                if isinstance(val.get('reset_label'),str): data['providers'][provider][name]['reset_label']=val['reset_label']
